...
Route::prefix('/kemahasiswaan')->name('kemahasiswaan.')->middleware(['auth', 'role:1'])->group(function () {
    ...
    Route::post('/verifikasi/{id}/{fileType}', [App\Http\Controllers\Kemahasiswaan\DataPengajuanController::class, 'verifikasi'])->name('data-pengajuan.verifikasi');
...
Route::prefix('/mahasiswa')->name('mahasiswa.')->middleware(['auth', 'role:0'])->group(function () {
    ...
    // Route untuk halaman upload ulang
    Route::get('/pengajuan/reupload', [App\Http\Controllers\Mahasiswa\PengajuanController::class, 'reupload'])
        ->name('pengajuan.reupload');

    // Route untuk menyimpan upload ulang
    Route::post('/pengajuan/store-reupload', [App\Http\Controllers\Mahasiswa\PengajuanController::class, 'storeReupload'])
        ->name('pengajuan.store-reupload');
});
